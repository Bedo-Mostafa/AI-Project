...
namespace EasyTransition
{
    ...
    {        
        ...
        private void Awake()
        {
            // safe singleton: keep one instance across scenes
            if (instance == null)
            {
                instance = this;
    ...
    var transitionComp = template.GetComponent<Transition>();
    transitionComp.transitionSettings = transitionSettings;

    // Keep the transition object alive through the scene load
    ...
    // Wait for the "in" animation
    yield return new WaitForSecondsRealtime(transitionTime);

    onTransitionCutPointReached?.Invoke();

    // Load the target scene (synchronous)
    SceneManager.LoadScene(sceneName);

    // Let the Transition component know the scene loaded so it can play the "out" animation
    transitionComp.OnSceneLoad(SceneManager.GetActiveScene(), LoadSceneMode.Single);

    // Wait for exit animation / destroy time
    yield return new WaitForSecondsRealtime(transitionSettings.destroyTime);

    onTransitionEnd?.Invoke();

    // Clean up the template (if Transition didn't destroy itself)
    if (template != null)
        Destroy(template);
    ...
    runningTransition = false;
        ...
        {
            // Don't run the duplicate check on instances that are being destroyed
            if (isDuplicate)
                yield break;
