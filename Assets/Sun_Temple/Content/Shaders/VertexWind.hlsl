...
half3 do_wind(half3 vertex_xyz, half2 color, half _WaveFreq, half _WaveHeight, float4x4 worldToObject){
    half phase_slow = _Time.y * _WaveFreq;
    half phase_med = _Time.y * 12 * _WaveFreq;
   
    half offset = (vertex_xyz.x + (vertex_xyz.z * 12)) * 2500;
    offset = mul(worldToObject, float4(offset, 0, 0, 0)).x;
    ...
    wind_xyz = mul(worldToObject, half4(wind_xyz, 0.0)).xyz;

    return wind_xyz;
}

half3 wind_simplified(half3 vertex_xyz, half4 color, half _WaveFreq, half _WaveHeight, half _WaveScale, float4x4 worldToObject){
    half phase_slow = _Time.y * _WaveFreq;
    half phase_med = _Time.y * 3 * _WaveFreq;
    half phase_fast = _Time.y * 5 * _WaveFreq;
